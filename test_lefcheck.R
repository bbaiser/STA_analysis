...
E1<-resid(manure , type="pearson")

plot(x=na.omit(manure),y=na.omit(E1) ) #plot residuals
qqnorm(E1)#qqplot
qqline(E1)#qqline
...
manure_arma <- lme(log10.manureN ~ Agro + log10.WatershedHa,
              random = ~ 1 | SUBEST_ID,
              correlation = corARMA(form = ~ 1 | SUBEST_ID/Year, p = 1, q = 0),
              sav_wsmodelTFOH)



summary(manure_arma)
r.squaredGLMM(manure_arma) 


#explore model residuals
E1<-resid(manure_arma , type="pearson")

plot(x=na.omit(manure_arma),y=na.omit(E1) ) #plot residuals
qqnorm(E1)#qqplot
qqline(E1)#qqline
